...
static int run() {
  ...
  while (!g_quit) {
    ...
    dx_gl_context *gl_context = DX_GL_CONTEXT(dx_gl_wgl_get_context());
    dx_context_set_viewport(DX_CONTEXT(gl_context), 0.f, 0.f, (float)canvas_width, (float)canvas_height);
    dx_context_clear_color(DX_CONTEXT(gl_context), 0.f / 255.f, 191.f / 255.f, 255.f / 255.f, 0.f); // color called "Capri" (0, 191, 255)
