...
static dx_asset_material* _parse_material(dx_adl_node* node, dx_adl_semantical_state* state) {
  ...
  // ambientColor?
  {
    ...
    if (child_node) {
      ...
      if (_parse_color_instance(child_node, state, &ambient_color_u8)) {
        goto END;
      }
  ...
  // ambientTexture?
  {
    ...
    dx_adl_node* child_node = dx_adl_node_map_get(node, name);
    if (child_node) {
      dx_asset_texture* ambient_texture = _parse_texture(child_node, state);
      if (!ambient_texture) {
        goto END;
      }
...
static dx_object* read(dx_adl_semantical_material_reader* self, dx_adl_node* node, dx_adl_semantical_state* state) {
  return DX_OBJECT(_parse_material(node, state));
}
