...
int dx_emit_msg_construct(dx_emit_msg* emit_msg, char const *p, size_t n) {
  dx_log("enter: dx_emit_msg_construct\n", sizeof("enter: dx_emit_msg_construct\n"));
  if (dx_msg_construct(DX_MSG(emit_msg))) {
    dx_log("leave: dx_emit_msg_construct\n", sizeof("leave: dx_emit_msg_construct\n"));
    return 1;
  }
...
void dx_emit_msg_destruct(dx_emit_msg* emit_msg) {
  dx_log("enter: dx_emit_msg_destruct\n", sizeof("enter: dx_emit_msg_destruct\n"));
  free(emit_msg->p);
  emit_msg->p = NULL;
  dx_msg_destruct(DX_MSG(emit_msg));
  dx_log("leave: dx_emit_msg_destruct\n", sizeof("leave: dx_emit_msg_destruct\n"));
}

dx_emit_msg* dx_emit_msg_create(char const *p, size_t n) {
  dx_log("enter: dx_emit_msg_create\n", sizeof("enter: dx_emit_msg_create\n"));
  dx_emit_msg* emit_msg = DX_EMIT_MSG(dx_object_alloc(sizeof(dx_emit_msg)));
  if (!emit_msg) {
    dx_log("leave: dx_emit_msg_create\n", sizeof("leave: dx_emit_msg_create\n"));
    return NULL;
  }
...
int dx_quit_msg_construct(dx_quit_msg* quit_msg) {
  dx_log("enter: dx_quit_msg_construct\n", sizeof("enter: dx_quit_msg_construct\n"));
  if (dx_msg_construct(DX_MSG(quit_msg))) {
    dx_log("leave: dx_quit_msg_construct\n", sizeof("leave: dx_quit_msg_construct\n"));
    return 1;
  }
...
dx_quit_msg* dx_quit_msg_create() {
  dx_log("enter: dx_quit_msg_create\n", sizeof("enter: dx_quit_msg_create\n"));
  dx_quit_msg* quit_msg = DX_QUIT_MSG(dx_object_alloc(sizeof(dx_quit_msg)));
  if (!quit_msg) {
    dx_log("leave: dx_quit_msg_create\n", sizeof("leave: dx_quit_msg_create\n"));
    return NULL;
  }
...
int dx_msg_queue_push(dx_msg_queue* msg_queue, dx_msg* msg) {
  dx_log("enter: dx_msg_queue_push\n", sizeof("enter: dx_msg_queue_push\n"));
  if (!msg_queue || !msg) {
    dx_set_error(DX_INVALID_ARGUMENT);
    dx_log("leave: dx_msg_queue_push\n", sizeof("leave: dx_msg_queue_push\n"));
    return 1;
  }
...
int dx_msg_queue_pop(dx_msg_queue* msg_queue, dx_msg** msg) {
  dx_log("enter: dx_msg_queue_pop\n", sizeof("enter: dx_msg_queue_pop\n"));
  if (msg_queue->size == 0) {
    *msg = NULL;
    dx_log("leave: dx_msg_queue_pop\n", sizeof("leave: dx_msg_queue_pop\n"));
    return 0;
  } else {
...
dx_msg_queue *dx_msg_queue_create() {
  dx_log("enter: dx_msg_queue_create\n", sizeof("enter: dx_msg_queue_create\n"));
  dx_msg_queue *msg_queue = malloc(sizeof(dx_msg_queue));
  if (!msg_queue) {
    dx_set_error(DX_ALLOCATION_FAILED);
    dx_log("allocation failed\n", sizeof("allocation failed\n"));
    dx_log("leave: dx_msg_queue_create\n", sizeof("leave: dx_msg_queue_create\n"));
    return NULL;
  }
  msg_queue->size = 0;
...
void dx_msg_queue_destroy(dx_msg_queue *msg_queue) {
  dx_log("enter: dx_msg_queue_destroy\n", sizeof("enter: dx_msg_queue_destroy\n"));
  if (msg_queue->elements) {
  ...
  free(msg_queue);
  msg_queue = NULL;
  dx_log("leave: dx_msg_queue_destroy\n", sizeof("leave: dx_msg_queue_destroy\n"));
}
