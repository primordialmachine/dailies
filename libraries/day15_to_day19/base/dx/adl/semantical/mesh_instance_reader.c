...
static dx_object* read(dx_adl_semantical_mesh_instance_reader*,
                       dx_adl_node* node,
...
static dx_object* read(dx_adl_semantical_mesh_instance_reader* self, dx_adl_node* node,
  ...
  // mesh
  {
    ...
    dx_string* received_type = dx_adl_semantical_read_type(child_node, names);
    ...
    mesh = (dx_asset_mesh*)dx_adl_semantical_reader_read(mesh_reader, child_node, state, names);
    if (!mesh) {
      goto END;
    }
  }
