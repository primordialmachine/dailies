...
static DX_ASSET_BRUSH* dx_adl_parse_brush(dx_adl_node* node, dx_adl_semantical_state* state, dx_adl_semantical_names* names) {
  ...
  type = dx_adl_semantical_read_type(node, names);
  if (!type) {
    return NULL;
  }
...
static dx_object* read(dx_adl_semantical_image_reader*,
                       dx_adl_node* node,
...
static dx_object* read(dx_adl_semantical_image_reader* self, dx_adl_node* node,
                       dx_adl_semantical_state* state, dx_adl_semantical_names* names) {
  return DX_OBJECT(_parse_image(node, state, names));
}
