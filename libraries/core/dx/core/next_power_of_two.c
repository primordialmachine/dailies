...
dx_n8 dx_next_power_of_two_n8(dx_n8 x) {
  ...
  size_t i = dx_count_leading_zeroes_n8(x);
  dx_n8 t = 1 << (8 - i);
  if (i == 0 && t != x) {
...
dx_n16 dx_next_power_of_two_n16(dx_n16 x) {
  ...
  size_t i = dx_count_leading_zeroes_n16(x);
  dx_n16 t = 1 << (16 - i);
  if (i == 0 && t != x) {
    dx_set_error(DX_OVERFLOW);
    return 0;
...
dx_n32 dx_next_power_of_two_n32(dx_n32 x) {
  ...
  size_t i = dx_count_leading_zeroes_n32(x);
  dx_n32 t = 1 << (32 - i);
  if (i == 0 && t != x) {
    dx_set_error(DX_OVERFLOW);
    return 0;
...
dx_n64 dx_next_power_of_two_n64(dx_n64 x) {
  ...
  size_t i = dx_count_leading_zeroes_n64(x);
  dx_n64 t = 1 << (64 - i);
  if (i == 0 && t != x) {
    dx_set_error(DX_OVERFLOW);
    return 0;
