...
/// @return The zero value on success. A non-zero value on failure.
static inline int _dx_impl_set(_dx_impl* self, dx_pointer_hashmap_key key, dx_pointer_hashmap_value value);
...
int dx_pointer_hashmap_set(dx_pointer_hashmap* self, dx_pointer_hashmap_key key, dx_pointer_hashmap_value value) {
  ...
  return _dx_impl_set(_DX_IMPL(self->pimpl), key, value);
}
