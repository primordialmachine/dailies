...
static dx_cbinding* create_cbinding(dx_val_mesh_instance* self) {
...
static int add_to_backend(dx_val_mesh_instance* self) {
...
int dx_val_mesh_instance_construct(dx_val_mesh_instance* self, DX_MAT4 world_matrix, dx_mesh* mesh) {
  ...
  dx_rti_type* _type = dx_val_mesh_instance_get_type();
  if (!_type) {
    return 1;
  }
  self->commands = NULL;
