...
static int mesh_loaded_callback(dx_asset_mesh* mesh) {
  dx_asset_mesh_operation* mesh_operation = 
    DX_ASSET_MESH_OPERATION
      (
        dx_asset_mesh_operations_set_vertex_colors_create()
      );
  if (dx_asset_mesh_operation_apply(mesh_operation, mesh)) {
...
static int on_startup_scene(dx_context* context) {
  g_scenes[0] = DX_SCENE(dx_mesh_viewer_scene_create("quadriliteral", &mesh_loaded_callback));
  if (!g_scenes[0]) {
    return 1;
  }
  g_scenes[1] = DX_SCENE(dx_mesh_viewer_scene_create("triangle", &mesh_loaded_callback));
  if (!g_scenes[1]) {
