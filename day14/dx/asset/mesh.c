...
int dx_asset_mesh_format(dx_asset_mesh* mesh, DX_VERTEX_FORMAT vertex_format, void** bytes, size_t* number_of_bytes) {
  ...
  case DX_VERTEX_FORMAT_POSITION: {
    void* p = malloc(mesh->number_of_vertices * sizeof(DX_VEC3));
    if (!p) {
      dx_set_error(DX_ALLOCATION_FAILED);
      return 1;
    }
    char* q = (char*)p;
    for (size_t i = 0, n = mesh->number_of_vertices; i < n; ++i) {
      *((DX_VEC3*)q) = mesh->vertices.xyz[i];
      q += sizeof(DX_VEC3);
    }
    *bytes = p;
    *number_of_bytes = mesh->number_of_vertices * (sizeof(DX_VEC3));
    return 0;
  } break;
  case DX_VERTEX_FORMAT_COLOR: {
    void* p = malloc(mesh->number_of_vertices * sizeof(DX_VEC4));
    if (!p) {
      dx_set_error(DX_ALLOCATION_FAILED);
      return 1;
    }
    char* q = (char*)p;
    for (size_t i = 0, n = mesh->number_of_vertices; i < n; ++i) {
      *((DX_VEC4*)q) = mesh->vertices.ambient_rgba[i];
      q += sizeof(DX_VEC4);
    }
    *bytes = p;
    *number_of_bytes = mesh->number_of_vertices * (sizeof(DX_VEC4));
    return 0;
  } break;
  case DX_VERTEX_FORMAT_POSITION_COLOR: {
    void* p = malloc(mesh->number_of_vertices * (sizeof(DX_VEC3) + sizeof(DX_VEC4)));
    if (!p) {
      dx_set_error(DX_ALLOCATION_FAILED);
      return 1;
    }
    ...
    *bytes = p;
    *number_of_bytes = mesh->number_of_vertices * (sizeof(DX_VEC3) + sizeof(DX_VEC4));
    return 0;
  } break;
  default: {
    return 1;
